...
def get_all_answer_options(data):
...
def get_questions_database():
    ...
    options = get_all_answer_options(data)
    answers = get_answers(options)
    answer_col = {"answers":answers}
# Save to csv
    qa_df = pd.concat([pd.DataFrame({"qid": [i for i in range(0, len(questions))]}) , questions[["Display","Answer Type"]] ,pd.DataFrame(answer_col)],axis=1)
    return qa_df

def save_questions_database_to_json(qa_df):
    qa_df.to_json('../data/ques_ans_clean.json')
...
if __name__ == '__main__':
    ...
    save_questions_database_to_json(qa_df)